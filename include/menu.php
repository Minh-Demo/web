...
<?php
	...
	?>
	<div class="navbar-inner" style="background-color: #33CCFF;">
		<div class="container" style="background-color: #33CCFF;">
			<nav class="navbar navbar-expand-lg navbar-light bg-light" >
				<div class="agileits-navi_search" >
					<form action="#" method="post">
						<select onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);" id="agileinfo-nav_search" name="agileinfo_search" class="border" required="">
							<option value="">Danh Mục Sản Phẩm</option>
							<?php
								while($row_category = mysqli_fetch_array($sql_category)){
							?>
							<option value="?quanly=danhmuc&id=<?php echo $row_category['category_id'] ?>"><?php echo $row_category['category_name']?></option>
							<?php
								}
							?>
						</select>
				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav ml-auto text-center mr-xl-5">
						...
						<li class="nav-item dropdown mr-lg-2 mb-lg-0 mb-2">
							...
							?>
							<a class="nav-link dropdown-toggle" href="?quanly=danhmuc&id=<?php echo $row_category_danhmuc['category_id'] ?> " role="button"  aria-haspopup="true" aria-expanded="false">
								<?php echo $row_category_danhmuc['category_name'] ?>
							</a>
							<!-- danh mục con  dropdown-->
							<div class="dropdown-menu">
								<?php
								while($row_brands = mysqli_fetch_array($sql_brands)){
								?>
								<a class="dropdown-item" href="?quanly=danhmuc&id=<?php echo $row_category_danhmuc['category_id'] ?>&brand_id=<?php echo $row_brands['brand_id'] ?>"><?php echo $row_brands['brand_name']?></a>
								<?php
								}
								?>
							</div>
