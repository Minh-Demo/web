...
if (isset($_GET['search_product'])) {
    $timkiem = $_GET['search_product'];
    $param_sort = "search_product=" . $timkiem . "&";
    ...
    if (isset($_GET['search_product']) && isset($_GET['thuonghieu'])) {
        $timkiem = $_GET['search_product'];
        $thuonghieu = $_GET['thuonghieu'];
        $sql_select_sanpham = mysqli_query($con, "SELECT * FROM tbl_sanpham,tbl_category,tbl_brands
                                        WHERE tbl_sanpham.category_id = tbl_category.category_id AND tbl_brands.brand_id = tbl_sanpham.brand_id AND category_name LIKE '%$timkiem%' AND tbl_sanpham.brand_id = $thuonghieu
                                        ORDER BY tbl_sanpham.sanpham_id DESC");
    }
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <select id="select-box" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                    ...
                    ?>
                        <option <?php if (isset($_GET['thuonghieu']) && $_GET['thuonghieu'] == $row_brands['brand_id']) { ?> selected <?php } ?> value="?<?= $param_sort ?>thuonghieu=<?php echo $row_brands['brand_id'] ?>"><?php echo $row_brands['brand_name'] ?></option>
                    <?php
                    }
                    ?>
                </select>
            ...
            <select id="select-box" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                ...
                ?>
                    <option <?php if (isset($_GET['danhmuc']) && $_GET['danhmuc'] == $row_danhmuc['category_id']) { ?> selected <?php } ?> value="?danhmuc=<?php echo $row_danhmuc['category_id'] ?>"><?php echo $row_danhmuc['category_name'] ?></option>
                <?php
                }
                ?>
            </select> -->
